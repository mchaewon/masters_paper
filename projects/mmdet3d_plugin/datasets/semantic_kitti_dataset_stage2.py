...
class SemanticKittiDatasetStage2(Dataset):
    def __init__(
        ...
        # ── Exp0: Event 설정 ──────────────────────────────────
        self.event_root = os.path.join(data_root, "events")
    ...
    def load_scans(self):
        ...
        for sequence in self.sequences:
            ...
            glob_path = os.path.join(
                self.data_root, "sequences_" + self.depthmodel + "_sweep"+ self.nsweep, sequence, "queries", "*." + self.query_tag
            )
    ...
    def load_event_as_rgb(self, sequence, frame_id):
        ...
        event_path = os.path.join(
            self.event_root, sequence, "image_0",f"{frame_id}.npy"
        )
    ...
    def get_input_info(self, sequence, frame_id):
        ...
        [Exp2] RGB only. Event는 _fuse_event_feats에서 별도 처리.

        Returns:
            torch.tensor: [N, 3, img_H, img_W]
        """
        seq_len = len(self.poses[sequence])
        image_list = []

        rgb_path = os.path.join(
            self.data_root, "sequences",
